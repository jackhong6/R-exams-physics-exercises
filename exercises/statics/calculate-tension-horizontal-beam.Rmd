...
tension <- beam_mass * 9.8 / 2 / sin(angle * pi / 180)
...
Question
========
A \SI{`r fmt(beam_length, digits=1)`}{m} long uniform beam of mass \SI{`r fmt(beam_mass, digits=1)`}{kg}, $QR$, is mounted by a hinge on a wall and held in a horizontal position by wire $PQ$, forming a \ang{`r angle`} angle at point $Q$ as shown in the figure. What is the magnitude of the tension in wire $PQ$?
...
Solution
========
The net torque about the hinge point $R$ must be zero because the beam is in static equilibrium. The force of gravity on the beam and the tension in the wire are the two forces applying a torque to the beam. Let $l$ be the length of the beam, $\theta$ be the angle at point $Q$, and $T$ be the tension in wire.

$$\tau_{net} = l T \sin(\theta) - \frac{lmg}{2} = 0$$
Solving for $T$, we get
$$T = \frac{mg}{2 \sin(\theta)} = \frac{(\SI{`r fmt(beam_mass, digits=1)`}{kg}) (\SI{9.8}{m/s^2})}{2 \sin(\ang{`r angle`})} = \SI{`r signif(tension, 2)`}{N}$$


Meta-information
================
exname: calculate tension horizontal beam
extype: schoice
