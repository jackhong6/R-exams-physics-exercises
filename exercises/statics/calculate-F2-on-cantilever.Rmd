...
sc <- num_to_schoice(F2, range(c(0.1, 2) * F2), delta = F2/10, format = FALSE)
...
tikz <- c("\\begin{tikzpicture}",
    ...
    sprintf("\\node[anchor=south] at (%f,2.3) {$\\mathbf{F_2}$};", d1 - d2 + 0.15),
    sprintf("\\draw[|<->|, thick] (0.15, 1.2) -- node[above, font=\\small] {\\SI{%g}{m}} (%f, 1.2);", d1 - d2, d1 - d2 + 0.15),
...
Question
========
A cantilever beam is held in static equilibrium by two vertical supports separated by \SI{`r d1 - d2`}{m}. The beam's mass is \SI{`r m`}{kg} and its centre of gravity (CG) is \SI{`r d2`}{m} from the second support. What is the magnitude of the force applied by the second support, $\mathbf{F_2}$?

```{r, echo = FALSE, results = "asis"}
include_tikz(tikz, name = "calculate-F2-on-cantilever", format = typ, library = c("angles", "quotes", "arrows.meta"), packages="siunitx")
```
...
$$d_1 F_1 - d_2 F_2 = 0$$
Solving the above system of linear equations for $F_2$ (by substitution or elimination) gives:

$$F_2 = \left(\frac{d_1}{d_1 - d_2} \right) mg = \left(\frac{`r d1`}{`r d1` - `r d2`} \right) (\SI{`r m`}{kg})(\SI{9.8}{N/kg}) = \SI{`r signif(F2, 2)`}{N}$$
...
Meta-information
================
exname: calculate F2 on cantilever
extype: schoice
